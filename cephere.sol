...
contract Cephere is Context, Ownable, ERC20  {
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CPH: Account has already the value of 'excluded'");
    ...
    function excludeFromMaxSellLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxSellTxLimit[account] != excluded, "CPH: Account has already the value of 'excluded'");
    ...
    function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxWalletLimit[account] != excluded, "CPH: Account has already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CPH: The main pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CPH: Automated market maker pair is already set to that value");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CPH: The router has already that address");
    ...
    function updateMainUniswapPair(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Pair), "CPH: The pair address has already that address");
    ...
    function setBuyTeamFee(uint8 newTeamFee) external onlyOwner {
        require(newTeamFee <=3 && newTeamFee >=0 ,"CPH: Buy team fee must be between 0 and 3");
    ...
    function setSellTeamFee(uint8 newTeamFee) external onlyOwner {
        require(newTeamFee <=5 && newTeamFee >=0 ,"CPH: Sell team fee must be between 0 and 5");
    ...
    function setMaxSellLimit(uint256 amount) external onlyOwner {
        require(amount >= 4000 && amount <= 4_000_000, "CPH: Amount must be bewteen 4000 and 4 000 000");
        maxSellLimit = amount *10**18;
        emit MaxSellLimitUpdated(amount);
    }

    function setMaxWalletLimit(uint256 amount) external onlyOwner {
        require(amount >= 40_000, "CPH: Amount must be greater than 40 000");
    ...
    function setLaunchTimestamp(uint256 newTimestamp) external onlyOwner {
        require(launchTimestamp > block.timestamp, "CPH: Changing the launch timestamp is not allowed if the launch has already started");
        launchTimestamp = newTimestamp;
    }

    // For adding presale addresses
    function addPresaleAddresses(address account) external onlyOwner {
        require(!_presaleAddresses[account],"CPH: This account is already added");
        _presaleAddresses[account] = true;
    }

    function setTeamWallet(address payable newWallet) external onlyOwner {
        require(newWallet != teamWallet, "CPH: The team wallet has already this address");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "CPH: Transfer from the zero address");
        require(to != address(0), "CPH: Transfer to the zero address");
        require(amount >= 0, "CPH: Transfer amount must be greater or equals to zero");

        bool isLaunched = block.timestamp >= launchTimestamp;
        // Only whitelisted addresses can send tokens before the launch
        if(!isLaunched) {
            require(_presaleAddresses[from], "CPH: This account is not a presale address");
        }
        ...
        if(!_isLiquefying) {
            if(isLaunched && isSellTransfer && from != address(uniswapV2Router) && !_isExcludedFromMaxSellTxLimit[from])
                require(amount <= maxSellLimit, "CPH: Amount exceeds the maxSellTxLimit");
            else if(!isSellTransfer && !isBuyTransfer && !_isExcludedFromMaxWalletLimit[to])
                require(balanceOf(to) + amount <= maxWalletLimit, "CPH: Amount exceeds the maxWalletLimit.");
            }
        ...
        // All transfers submitted to the tax, are subject to the cooldown system as well
            ...
            if(coolDownEnabled && !isBuyTransfer && !_presaleAddresses[from]){
                uint256 timePassed = block.timestamp - _lastTimeTx[from];
                 require(timePassed >= coolDownTime, "CPH: The cooldown is not finished, please retry the transfer later");
             }
        }
        ...
        if(takeFee) {
            ...
            if(isBuyTransfer){
                ...
                if(!_isExcludedFromMaxWalletLimit[to]) require(balanceOf(to) + amountWithFees <= maxWalletLimit, "CPH: Amount exceeds the maxWalletLimit.");
    ...
    function withdrawStuckBNB(address payable to) external onlyOwner {
        require(address(this).balance > 0, "CPH: There are no BNBs in the contract");
